...
# Experimental data
# path = "data/gst-600/"
# test_date = "2020-10-05 16-04-53[x]/fmt/"
# Simulated  data
path = "data/gst-600/magnetic/sim/"
test_date = "2020-10-26 20-04-20/"
...
# z = np.append(z, [80*1e-3, 120*1e-3, 160*1e-3, 200*1e-3])
# Nz = z.size
...
# process data
def find_step(t, step_response, zi):
	n = len(step_response)
	delta = np.zeros(n)
	...
	if debug:
		...
		print("k=", k)
		print("k2=", k2)
		# stp = Monitor([t], [step_response], "Step response", "step[]", "time[s]", sig_name = ["raw"])
		# stp.plot()
		# stp.show()
	if k == 0:
...
for i in range(0, Nz):
	...
	[m_offset, m_steady, t_steady] = find_step(time[i], magF[i], z[i])
...
if SAVE_DATA:
	...
	file = Path(path+test_date+"results[B-m-mean-std]["+str(int(z[0]*1e3))+"-"+str(int(z[-1]*1e3))+"mm].csv")
	df.to_csv(file)
